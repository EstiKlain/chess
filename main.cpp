...
// https://github.com/EstiKlain/chess.git
...
void handleClick(GameState &st, int x, int y)
{
    ...
    if (!isEmpty(token))
    {
        ...
        if (side < 0 || side >= (int)st.selections.size())
            return;
