...
#### Server-Iteration 2 — second-opinion CR follow-up (done, verified green)

The user had a separate chat act as an independent reviewer on the Iteration 2 diff and brought back 6 findings. One was a false alarm: `tests/unit/server/MakeMoveUseCase.tests.cpp` already existed and covered exactly what was asked for (legal move → `MoveApplied` + `STATE_UPDATE` to both; illegal move → `ILLEGAL_MOVE` to sender only) — the other reviewer just wasn't shown the file. The other 5 were real, triaged as follows:

- **`requestId` was parsed but never propagated** (`BusEvent` had no `requestId` field; `protocol::envelope()` always hardcoded `""`) — **fixed.** `BusEvent` now carries `requestId`; `MessageRouter` passes `envelope.requestId` through to it; `protocol::envelope`/`errorEnvelope` gained overloads that take a `requestId`. **Semantics decided with the user:** `requestId` only ever flows back to the exact connection that sent the original request — `MakeMoveUseCase::fanOutStateUpdate` gives the mover their own `requestId` back, but the *other* player's `STATE_UPDATE` (a side effect they never asked for) always gets `requestId: ""`. Echoing the mover's id to a bystander would wrongly imply they have a pending request by that name.
- **`MALFORMED_PAYLOAD`/`TABLE_FULL` missing from the plan doc's error-code table** — **fixed, documentation only.** Added both to `docs/kungfu_chess_server_plan (1).md`'s table with a one-line reason each. Explicitly **rejected** the reviewer's alternative suggestion to merge `MALFORMED_PAYLOAD` back into `INTERNAL_ERROR` — that split was deliberate (see above) for a real distinction (bad input from the peer vs. an internal state that should never happen), reverting it would undo that for no benefit.
- **Duplication between `handleMove`/`handleJump`** — **fixed, but not with the reviewer's proposed generic `template<typename Result> handleResult(...)`.** That template only covers the identical tail (accepted-check → error/fan-out) and would still need a second layer of indirection (a lambda or another template parameter) to also cover the middle section (DTO parsing + the engine call), which genuinely differs in type between `MoveDto`/`requestMove` and `JumpDto`/`requestJump`. Chose two small non-template private helpers instead: `lookupBoundSession(connectionId, requestId)` (the identical head - binding lookup + the "not bound" error path) and `finishRequest(connectionId, requestId, session, accepted, reason)` (the identical tail - `MoveResult`/`JumpResult` already share the exact same `{accepted, reason}` shape, so no generic parameter is needed, just two plain fields). The middle section (DTO parsing, the actual engine call) stays as two separate, readable methods - less machinery for the same amount of duplication removed.
- **`fanOutStateUpdate` did a lookup, then a second lookup per recipient just to read the color back** — **fixed.** `ConnectionManager::connectionIdsFor(session) -> vector<string>` replaced by `connectionsFor(session) -> vector<pair<string, ConnectionBinding>>`, returning the color alongside the id from the same pass over the map. Removes the redundant `sessionFor()` call per recipient inside the fan-out loop.
- **Detached tick thread has no shutdown path** — **left as-is, correctly scoped as a tracked risk, not an Iteration 2 blocker.** Already documented in both the code comment and this file as something to revisit once Iteration 6 (SQLite) makes an abrupt process exit actually dangerous (an open DB transaction) — no action needed before then.

All 18 server tests (now 74 assertions, up from 65 - added explicit `requestId` correlation checks) and all 188 `chess_tests` pass.
